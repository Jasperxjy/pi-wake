...
const child = spawn(process.execPath, [piCli, "--mode", "rpc", "--offline", "--approve", "--no-session", "--no-extensions", "--extension", extensionEntry], {
	...
	try {
		...
		assert.ok(widget.widgetLines!.some((line) => /timer\s+RPC probe\s+in \d+[sm]/.test(line)), `timer table row present: ${JSON.stringify(widget.widgetLines)}`);
		assert.ok(!widget.widgetLines!.some((line) => /^T timer/.test(line)), "no separate legend line: the type words are self-explanatory");
